...
public class LSM_Toolbox implements PlugIn, MacroExtension {
	...
	private final ExtensionDescriptor[] extensions = {
		ExtensionDescriptor.newDescriptor("lsmXML", this, ARG_STRING + ARG_OUTPUT),
		ExtensionDescriptor.newDescriptor("getTStamps", this, ARG_STRING +
			ARG_OUTPUT),
		ExtensionDescriptor.newDescriptor("getZStamps", this, ARG_STRING +
			ARG_OUTPUT),
		ExtensionDescriptor.newDescriptor("getLStamps", this, ARG_STRING +
			ARG_OUTPUT),
		ExtensionDescriptor.newDescriptor("getEvents", this, ARG_STRING +
			ARG_OUTPUT),
		ExtensionDescriptor.newDescriptor("lsmOpen", this, ARG_STRING) };
	...
	public void run(final String args) {
		...
		if (IJ.versionLessThan("1.41a")) return;

		String fileName = "";
		final String macroOptions = Macro.getOptions();
		if (IJ.macroRunning()) if (macroOptions.trim().equalsIgnoreCase("ext")) {
			Functions.registerExtensions(this);
			return;
		}
		if (!args.equals("")) fileName = getMacroOption("file=", args);
		if (macroOptions != null && (!macroOptions.equals(""))) fileName =
			getMacroOption("file=", macroOptions).trim();
